...
export default function PricingCard({
  ...
  const percentPriceDiscount =
    isDiscounted && oldPrice
      ? Math.round(
          ((parsePrice(oldPrice) - parsePrice(price)) / parsePrice(oldPrice)) *
            100,
        )
      : 0;
  ...
  if (badgeText) {
    badges.push({
      text: badgeText,
      variant: isBestSeller ? "bestSeller" : "popular",
    });
  } else {
    if (isBestSeller)
      badges.push({ text: "Mais vendido", variant: "bestSeller" });
    if (isPopular) badges.push({ text: "Mais popular", variant: "popular" });
  }

  let cardStyle =
    "bg-zinc-900/40 border border-white/5 hover:border-white/20 hover:shadow-2xl hover:bg-zinc-900/60";
  if (isBestSeller) {
    cardStyle =
      "glass ring-2 ring-orange-500 shadow-[0_0_40px_-10px_rgba(239,68,68,0.4)]";
  } else if (isDiscount6Meses) {
    cardStyle =
      "glass ring-2 ring-red-500 shadow-[0_0_40px_-10px_rgba(239,68,68,0.4)]";
  } else if (isPopular) {
    cardStyle =
      "glass ring-2 ring-purple-500 shadow-[0_0_40px_-10px_rgba(168,85,247,0.4)]";
  }
  ...
  return (
    ...
    >
      ...
      <div className="mb-8" style={{ transform: "translateZ(30px)" }}>
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex items-baseline gap-1 flex-wrap">
            <span className="text-4xl md:text-5xl lg:text-4xl xl:text-5xl font-black text-white tracking-tight">
              R$ {price}
            </span>
      ...
      <ul
        className="flex-grow space-y-4 mb-10 relative z-10"
      ...
      <motion.a
        ...
        className={cn(
          ...
          isBestSeller
            ? "bg-orange-600 text-white shadow-[0_10px_20px_-10px_rgba(234,88,12,0.8)] hover:bg-orange-500"
            : isDiscount6Meses
              ? "bg-red-600 text-white shadow-[0_10px_20px_-10px_rgba(239,68,68,0.8)] hover:bg-red-500"
              : isPopular
                ? "bg-purple-500 text-white shadow-[0_10px_20px_-10px_rgba(168,85,247,0.8)] hover:bg-purple-400"
      ...
      >
        ...
        <SiWhatsapp
          ...
          color={isBestSeller ? "orange" : isPopular ? "purple-800" : "white"}
        />
